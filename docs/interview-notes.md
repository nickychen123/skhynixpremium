...
- **Follow-up (twelve precedents and a control group)**: extended to a paper's fifteen-company dataset. Four ADRs are delisted with no free history; listed them as unavailable rather than dropping them. The first pass on the Brazilian and Mexican names produced impossible series (Itaú at an 18% discount for six years). Since a freely convertible ADR cannot hold a step-shaped premium, every step had to be an artefact; reconciling the split-event lists on both listings showed each one was a bonus issue the vendor recorded on one side only, and the corrections reproduced the published Wipro figures and put all eight convertible names at zero. The control group is what makes the argument: same method, same years, eight at parity and four with premiums.
...
5. ADR history tab: the summary table first (four restricted names with premiums, eight convertible names at zero), then Infosys (above 30% for six years, then parity from 2009), TSMC (never settled at zero), and the "since listing" chart with SKHY's first two months against Infosys and Wipro. The dashed line is SKHY today.
