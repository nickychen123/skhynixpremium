...
tab_live, tab_bt, tab_calc, tab_about = st.tabs(["Monitor", "Carry backtest", "Unwind calculator", "About"])
...
def live_panel() -> None:
    ...
    with h1:
        ...
        st.caption(f"SKHY {usd(d['b_mark'])} × 10 = {usd(d['b_mark'] * 10 if ok(d['b_mark']) else None)}  vs  "
    ...
    t[0].caption(f"SKHY {usd(d['b_oracle'])} · SKHYNIX {usd(d['a_oracle'], 1)} · oracles last moved {age(oa)} / {age(ob)} ago")
...
with tab_bt:
    ...
    if bs:
        ...
        st.caption(f"{bs['start']} → {bs['end']} · {bs['hours']} hours · long-leg notional at entry {usd(bs['notional_usd'], 0)}. "
